...
PurePursuitNode::PurePursuitNode()
    : private_nh_("~"), LOOP_RATE_(10), curvature_MIN_(1 / 9e10), is_waypoint_set_(false), is_pose_set_(false), current_linear_velocity_(0), command_linear_velocity_(0), next_waypoint_number_(-1), lookahead_distance_(0), is_last_point(false), is_in_cross(false), is_not_lane(true), pre_index(-1), search_start_index(0), clearest_points_index(-1), search_radius(2), cross_in(false), cross_out(false), almost_reach(false), lock_index(-1), save_index(-1), lane_speed_limit(1.5 * 3.6), cross_speed_limit(0.5 * 3.6)
{
...
void PurePursuitNode::initForROS()
{
    ...
    private_nh_.param("minimum_lookahead_distance_", minimum_lookahead_distance_, double(2));
    private_nh_.param("const_lookahead_distance_", const_lookahead_distance_, double(5));
    private_nh_.param("const_velocity_", const_velocity_, double(5.4)); //km/h
    private_nh_.param("is_const_lookahead_dis", is_const_lookahead_dis_, true);
    private_nh_.param("is_const_speed_command_", is_const_speed_command_, false);
